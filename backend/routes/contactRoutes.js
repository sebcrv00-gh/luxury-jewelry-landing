...
router.post('/', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    await Ticket.create(userId, {
      asunto: `Consulta web de ${name}`,
      mensaje: `MENSAJE CORE:\n${message}\n\n--- INFO ADICIONAL ---\nEmail: ${email}\nTeléfono: ${phone}`,
      orden_id: null
    });
    ...
    const mailOptions = {
      ...
      subject: `Nuevo Mensaje en Panel Admin: ${name}`,
      text: `Tienes un mensaje de ${name}. Puedes gestionarlo en tu Panel de Administración.\n\nMensaje: ${message}`,
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; border: 1px solid #c9a84c; border-radius: 8px;">
          <h2 style="color: #c9a84c;">Nuevo Mensaje Recibido</h2>
          <p>Has recibido una consulta de <strong>${name}</strong>.</p>
